...
class CandidateOut(BaseModel):
    """One row in the unified candidate pool — covers both outbound
    prospects (cold-discovered via LinkedIn) and inbound applicants
    (CSV uploaded from Luma). Normalized so the frontend can render one
    table with a Source column instead of two separate screens."""
    id: str           # 'p-123' for prospects, 'a-456' for applicants
    source: str       # 'outbound' | 'inbound'
    name: str
    role: Optional[str]
    company: Optional[str]
    email: Optional[str]
    ...
    recommendation: Optional[str]  # accept | maybe | reject | needs_review
    status: Optional[str]          # outbound: approved/contacted/rsvp ; inbound: human_decision
    one_sentence_summary: Optional[str]
    fit_reason: Optional[str]
    archetype: Optional[str]


@router.get("/{event_id}/candidates", response_model=list[CandidateOut])
def list_candidates(
    event_id: int,
    db: Session = Depends(get_db),
    user: models.User = Depends(current_user),
):
    """Unified pool : outbound prospects + inbound applicants combined,
    normalized to a single shape. Sorted by fit_score descending so the
    operator sees their best matches at the top regardless of channel.

    Read-only : individual mutations (mark RSVP, set decision, send
    invite) still go through each source's per-source endpoints. This
    endpoint is for the unified review UI."""
    ev = get_owned_event(event_id, user, db)
    out: list[CandidateOut] = []

    # Outbound prospects → normalize
    for p in ev.prospects:
        out.append(CandidateOut(
            ...
            role=p.role,
            company=p.company,
            email=None,  # prospects don't carry an email field
            linkedin_url=p.linkedin_url,
            fit_score=p.fit_score,
            confidence_score=None,
            # Prospects don't run through the triage recommendation bucket
            # logic — surface their status instead so the UI can show
            # "approved" / "contacted" / "rsvp" as a quasi-recommendation.
            recommendation=None,
            status=p.status,
    ...
    # Inbound applicants → normalize via their evaluation row
    for a in ev.applicants:
        e = a.evaluation
        d = a.decision
        out.append(CandidateOut(
            id=f"a-{a.id}",
            ...
            fit_score=(e.fit_score if e else None),
            confidence_score=(e.confidence_score if e else None),
            recommendation=(e.recommendation if e else None),
            status=(d.human_decision if d else None),
            one_sentence_summary=(e.one_sentence_summary if e else None),
            fit_reason=None,
            archetype=(e.archetype if e else None),
        ))

    # Sort by fit_score desc, treating None as -1 so unscored rows sink.
    out.sort(key=lambda c: -(c.fit_score if c.fit_score is not None else -1))
    return out
